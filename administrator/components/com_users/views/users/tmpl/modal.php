	...
	<fieldset class="filter">
		<div id="filter-bar" class="btn-toolbar">
			...
			<div class="btn-group pull-left">
				...
				<button type="button" class="btn" onclick="if (window.parent) window.parent.jSelectUser('', '<?php echo JText::_('JLIB_FORM_SELECT_USER'); ?>', '<?php echo $this->escape($field); ?>');"><?php echo JText::_('JOPTION_NO_USER'); ?></button>
			</div>
			...
			<tbody>
				...
				foreach ($this->items as $item) : ?>
					<tr class="row<?php echo $i % 2; ?>">
						<td>
							<a class="pointer" onclick="if (window.parent) window.parent.jSelectUser('<?php echo $item->id; ?>', '<?php echo $this->escape(addslashes($item->name)); ?>', '<?php echo $this->escape($field); ?>');">
								<?php echo $item->name; ?>
							</a>
						</td>
						<td align="center">
