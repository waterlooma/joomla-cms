...
class ConfigModelModules extends ConfigModelForm
{
	...
	public function getPositions()
	{
		...
		if (file_exists($path))
		{
			...
			if (isset($xml->positions[0]))
			{
				...
				foreach ($xml->positions[0] as $position)
				{
					...
					$currentTemplatePositions[] = ConfigModelModules::createOption($value, JText::_($text) . ' [' . $value . ']');
				}
			}
		}
		...
		$option = ConfigModelModules::createOption();
		$templateGroups[''] = ConfigModelModules::createOptionGroup('', array($option));

		$templateGroups[$templateName] = ConfigModelModules::createOptionGroup($templateName, $currentTemplatePositions);

		// Add custom position to options
		$customGroupText = JText::_('COM_MODULES_CUSTOM_POSITION');

		$editPositions = true;
		$customPositions = ConfigModelModules::getActivePositions(0, $editPositions);
		$templateGroups[$customGroupText] = ConfigModelModules::createOptionGroup($customGroupText, $customPositions);
